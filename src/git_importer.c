...
static FILE *open_git_stream(const char *repo_path, const char *const arguments[], pid_t *child_pid) {
    int pipe_fds[2];
    if (pipe(pipe_fds) != 0) return NULL;
    pid_t pid = fork();
    if (pid < 0) { close(pipe_fds[0]); close(pipe_fds[1]); return NULL; }
    if (pid == 0) {
        close(pipe_fds[0]);
        ...
        for (int i = 0; arguments[i] && index < 15; i++) argv[index++] = (char *)arguments[i];
        argv[index] = NULL;
        execvp("git", argv);
        _exit(127);
    }
    ...
    if (!stream) { close(pipe_fds[0]); waitpid(pid, NULL, 0); return NULL; }
    *child_pid = pid;
    return stream;
}
...
static bool close_git_stream(FILE *stream, pid_t child_pid) {
    fclose(stream);
    int status = 0;
    return waitpid(child_pid, &status, 0) == child_pid && WIFEXITED(status) && WEXITSTATUS(status) == 0;
}
...
static bool resolve_repository_root(const char *path, char *root, size_t size) {
    ...
    if (!read_ok || !command_ok) return false;
    root[strcspn(root, "\r\n")] = '\0';
    return root[0] != '\0';
}

// 辅助函数：判断文件路径中是否包含指定字符序列
static bool contains(const char *path, const char *needle) { return strstr(path, needle) != NULL; }
...
static const char *classify_path(const char *path) {
    ...
    if (contains(path, "node_modules/") || contains(path, "vendor/") ||
        ...
        (extension && strcmp(extension, ".lock") == 0)) return "generated";
    if (contains(path, "test/") || contains(path, "tests/") ||
        contains(path, "__tests__/") || contains(path, "spec/") ||
        strstr(path, "_test.") || strstr(path, ".test.") || strstr(path, ".spec.")) return "test";
    ...
    if (extension) {
        const char *code[] = {".c",".h",".cc",".cpp",".hpp",".m",".mm",".swift",".go",".rs",
            ".java",".kt",".kts",".py",".rb",".php",".js",".jsx",".ts",".tsx",
            ".vue",".svelte",".css",".scss",".less",".html",".sql",".sh",".zsh"};
        for (size_t i = 0; i < sizeof(code) / sizeof(code[0]); i++)
            if (strcmp(extension, code[i]) == 0) return "business";
    }
    return "other";
}
...
static bool insert_commit(sqlite3 *db, const char *repo, const char *hash,
                          const char *authored_at, const char *author, bool *inserted) {
    sqlite3_stmt *stmt = NULL;
    const char *sql = "INSERT OR IGNORE INTO git_commits(repo_path,commit_hash,authored_at,author_name) VALUES(?1,?2,?3,?4)";
    bool ok = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) == SQLITE_OK;
    if (ok) {
        sqlite3_bind_text(stmt,1,repo,-1,SQLITE_TRANSIENT); sqlite3_bind_text(stmt,2,hash,-1,SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt,3,authored_at,-1,SQLITE_TRANSIENT); sqlite3_bind_text(stmt,4,author,-1,SQLITE_TRANSIENT);
...
static bool insert_file(sqlite3 *db, const char *repo, const char *hash, const char *file_path,
                        long added, long deleted, bool *inserted) {
    sqlite3_stmt *stmt = NULL;
    const char *sql = "INSERT OR IGNORE INTO git_commit_files"
        "(repo_path,commit_hash,file_path,category,lines_added,lines_deleted) VALUES(?1,?2,?3,?4,?5,?6)";
    bool ok = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) == SQLITE_OK;
    if (ok) {
        sqlite3_bind_text(stmt,1,repo,-1,SQLITE_TRANSIENT); sqlite3_bind_text(stmt,2,hash,-1,SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt,3,file_path,-1,SQLITE_TRANSIENT); sqlite3_bind_text(stmt,4,classify_path(file_path),-1,SQLITE_STATIC);
        sqlite3_bind_int64(stmt,5,added); sqlite3_bind_int64(stmt,6,deleted);
        ok = sqlite3_step(stmt) == SQLITE_DONE;
        *inserted = ok && sqlite3_changes(db) > 0;
    }
    sqlite3_finalize(stmt);
...
static bool insert_git_fingerprint(sqlite3 *db, const char *repo, const char *hash,
                                   const char *file_path, long ordinal,
                                   const unsigned char *content, size_t content_length) {
    if (content_length == 0) return true;
    if (content[content_length - 1] == '\r') content_length--;
    if (content_length == 0) return true;
    char fingerprint[65];
    sha256_hex(content, content_length, fingerprint);
    sqlite3_stmt *stmt = NULL;
    const char *sql = "INSERT OR IGNORE INTO git_line_fingerprints"
...
static bool import_git_fingerprints(sqlite3 *db, const char *repo_root) {
    const char *arguments[] = {"log", "--all", "--reverse", "--no-renames",
        "--no-ext-diff", "--format=__AGENTSTAT_COMMIT__%H", "--patch", "--unified=0", NULL};
    pid_t child_pid;
    ...
    while (ok && fgets(line, sizeof(line), stream)) {
        ...
        if (strncmp(line, "__AGENTSTAT_COMMIT__", 20) == 0) {
            ...
            current_file[0] = '\0';
                   ...
                   strncmp(line, "+++", 3) != 0) {
            ok = insert_git_fingerprint(db, repo_root, current_hash, current_file, ordinal++,
                                        (const unsigned char *)(line + 1), length - 1);
        }
    }
    if (!close_git_stream(stream, child_pid)) ok = false;
    return ok;
}

// 核心对外接口：扫描给定路径寻找 Git 仓库，导入包含日志、文件统计及指纹在内的全部变迁记录
bool sync_git_repository(const char *path, GitImportResult *result) {
    if (!path || !result) return false;
    memset(result, 0, sizeof(*result));
    char repo_root[PATH_MAX];
    if (!resolve_repository_root(path, repo_root, sizeof(repo_root)) || !initialize_storage()) return false;
    ...
    if (ok) ok = sqlite3_prepare_v2(db, "INSERT INTO git_repositories(repo_path) VALUES(?1) "
        "ON CONFLICT(repo_path) DO UPDATE SET synced_at=CURRENT_TIMESTAMP", -1, &repo_stmt, NULL) == SQLITE_OK;
    if (ok) { sqlite3_bind_text(repo_stmt,1,repo_root,-1,SQLITE_TRANSIENT); ok = sqlite3_step(repo_stmt) == SQLITE_DONE; }
    sqlite3_finalize(repo_stmt);

    const char *arguments[] = {"log", "--all", "--reverse", "--no-renames",
        "--format=__AGENTSTAT_COMMIT__%H%x09%aI%x09%an", "--numstat", NULL};
    pid_t child_pid;
    FILE *stream = ok ? open_git_stream(repo_root, arguments, &child_pid) : NULL;
    if (ok && !stream) ok = false;
    char line[16384];
    char current_hash[128] = "";
    while (ok && fgets(line, sizeof(line), stream)) {
        line[strcspn(line, "\r\n")] = '\0';
        if (strncmp(line, "__AGENTSTAT_COMMIT__", 20) == 0) {
            ...
            if (!timestamp) { ok = false; break; }
            *timestamp++ = '\0';
            char *author = strchr(timestamp, '\t');
            if (!author) { ok = false; break; }
            *author++ = '\0';
            snprintf(current_hash, sizeof(current_hash), "%s", hash);
            bool inserted = false;
            ok = insert_commit(db, repo_root, current_hash, timestamp, author, &inserted);
            result->commits_scanned++;
            if (inserted) result->commits_imported++;
        } else if (current_hash[0] && line[0]) {
            char *added_text = line;
            char *deleted_text = strchr(added_text, '\t');
            if (!deleted_text) continue;
            *deleted_text++ = '\0';
            char *file_path = strchr(deleted_text, '\t');
            if (!file_path) continue;
            *file_path++ = '\0';
            long added = strcmp(added_text, "-") == 0 ? 0 : strtol(added_text, NULL, 10);
            long deleted = strcmp(deleted_text, "-") == 0 ? 0 : strtol(deleted_text, NULL, 10);
            bool inserted = false;
            ok = insert_file(db, repo_root, current_hash, file_path, added, deleted, &inserted);
            if (inserted) result->files_imported++;
        }
    }
    if (stream && !close_git_stream(stream, child_pid)) ok = false;
    if (ok) ok = import_git_fingerprints(db, repo_root);
