...
static void print_sync_result(const char *name, const CodexSyncResult *result) {
  printf("%-12s %ld session files, %ld usage events, %ld tool calls, %ld code "
         "changes (%ld files, %ld failed).\n",
...
int parse_and_execute_cli(int argc, char *argv[]) {
  ...
  if (strcmp(subcommand, "import-codex") == 0) {
    ...
    printf("Imported Codex session: %ld usage events, %ld tool calls, %ld code "
           "changes (%ld lines scanned).\n",
  ...
  if (strcmp(subcommand, "sync-codex") == 0) {
    ...
    if (!path)
      path = source_path("AGENTSTAT_CODEX_DIR", ".codex/sessions", default_path,
                         sizeof(default_path));
    CodexSyncResult result;
    bool ok = sync_codex_directory(path, &result);
    printf("Scanned %ld files and %ld lines; imported %ld usage events, %ld "
           "tool calls and %ld code changes.\n",
           result.files_scanned, result.lines_read,
           result.usage_events_imported, result.tool_calls_imported,
  ...
  if (strcmp(subcommand, "import-claude") == 0) {
    if (argc < 3) {
      fprintf(stderr, "Usage: %s import-claude <session.jsonl>\n", argv[0]);
      return 1;
    }
    CodexImportResult result;
    if (!import_claude_jsonl(argv[2], &result)) {
      fprintf(stderr, "Failed to import Claude JSONL: %s\n", argv[2]);
      return 1;
    }
    printf(
        "Imported Claude session: %ld usage events, %ld tool calls, %ld code "
        "changes (%ld lines scanned; %ld duplicate usage fragments skipped).\n",
        result.usage_events_imported, result.tool_calls_imported,
        result.code_changes_imported, result.lines_read,
        result.duplicate_usage_events_skipped);
    return 0;
  ...
  if (strcmp(subcommand, "sync-claude") == 0) {
    ...
    const char *path =
        argc >= 3 ? argv[2]
                  : source_path("AGENTSTAT_CLAUDE_DIR", ".claude/projects",
                                default_path, sizeof(default_path));
    CodexSyncResult result;
    bool ok = sync_claude_directory(path, &result);
    print_sync_result("Claude", &result);
    return ok ? 0 : 1;
  }

  // "import-antigravity" (或 "import-agy") 命令：导入 Antigravity 工具的 JSONL
  // 会话记录
  if (strcmp(subcommand, "import-antigravity") == 0 ||
      strcmp(subcommand, "import-agy") == 0) {
    if (argc < 3) {
      fprintf(stderr, "Usage: %s import-antigravity <transcript.jsonl>\n",
              argv[0]);
      return 1;
    }
    CodexImportResult result;
    if (!import_antigravity_jsonl(argv[2], &result)) {
      fprintf(stderr, "Failed to import Antigravity JSONL: %s\n", argv[2]);
      return 1;
    }
    printf("Imported Antigravity session: %ld tool calls and %ld code changes "
           "(%ld lines scanned).\n",
           result.tool_calls_imported, result.code_changes_imported,
           result.lines_read);
    return 0;
  ...
  // "sync-antigravity" (或 "sync-agy") 命令：同步并解析 Antigravity
  // 系统目录下所有的交互与代码修改数据
  if (strcmp(subcommand, "sync-antigravity") == 0 ||
      strcmp(subcommand, "sync-agy") == 0) {
    char default_path[1024];
    const char *path = argc >= 3
                           ? argv[2]
                           : source_path("AGENTSTAT_ANTIGRAVITY_DIR",
                                         ".gemini/antigravity-cli",
                                         default_path, sizeof(default_path));
    CodexSyncResult result;
    bool ok = sync_antigravity_directory(path, &result);
    print_sync_result("Antigravity", &result);
    return ok ? 0 : 1;
  }
  ...
  if (strcmp(subcommand, "usage") == 0) {
    ...
    if (source_count > 0) {
      ...
      for (int i = 0; i < source_count; i++)
        printf("  %-12s %ld sessions, %ld model calls, %ld tools, %ld code "
               "changes\n",
  ...
  // "code" 命令：加载并展示由 AI
  // 引发的代码变更统计数据（变动的文件、增删行数以及代码用途的分类）
  ...
  if (strcmp(subcommand, "sync-git") == 0) {
    ...
    printf(
        "Scanned %ld Git commits; imported %ld commits and %ld file changes.\n",
        result.commits_scanned, result.commits_imported, result.files_imported);
    return 0;
  }

  // "git-stats" 命令：从本地存储加载并显示已同步 Git
  // 数据的情况，细化为各类代码的修改统计
  ...
  // "attribution" 命令：评估由 AI
  // 工具推荐的代码段有多少被实际留存在代码库中（即代码采纳率）
  if (strcmp(subcommand, "attribution") == 0) {
    ...
    printf("Tests:                %ld / %ld\n", stats.test_accepted_lines,
           stats.test_candidate_lines);
    printf("Documentation:        %ld / %ld\n",
           stats.documentation_accepted_lines,
  ...
  if (strcmp(subcommand, "pricing") == 0) {
    if (argc < 3) {
      fprintf(stderr, "用法: %s pricing <set|list>\n", argv[0]);
      return 1;
    }
    if (strcmp(argv[2], "list") == 0) {
      AgentModelStats rows[256];
      int count = load_model_pricing(rows, 256);
      if (count < 0)
        return 1;
      printf("%-14s %-36s %12s %12s %12s %12s\n", "Agent", "模型", "普通输入",
             "缓存读取", "缓存写入", "输出");
      for (int i = 0; i < count; i++)
        printf("%-14.14s %-36.36s %12.4f %12.4f %12.4f %12.4f\n",
               rows[i].source, rows[i].model, rows[i].input_rate,
               rows[i].cache_read_rate, rows[i].cache_write_rate,
               rows[i].output_rate);
      if (count == 0)
        printf("尚未配置模型价格。\n");
      return 0;
    ...
    if (strcmp(argv[2], "set") == 0) {
      const char *source = NULL, *model = NULL;
      double input = 0, read = 0, write = 0, output = 0;
      int flags = 0;
      for (int i = 3; i < argc; i++) {
        if (strcmp(argv[i], "--source") == 0 && i + 1 < argc) {
          ...
          flags |= 2;
                   ...
                   parse_rate(argv[i + 1], &input)) {
          i++;
          flags |= 4;
        } else if (strcmp(argv[i], "--cache-read") == 0 && i + 1 < argc &&
                   parse_rate(argv[i + 1], &read)) {
          i++;
          flags |= 8;
        } else if (strcmp(argv[i], "--cache-write") == 0 && i + 1 < argc &&
                   parse_rate(argv[i + 1], &write)) {
          i++;
          flags |= 16;
        } else if (strcmp(argv[i], "--output") == 0 && i + 1 < argc &&
                   parse_rate(argv[i + 1], &output)) {
          i++;
          flags |= 32;
        } else {
          fprintf(stderr, "未知或缺少参数: %s\n", argv[i]);
          return 1;
        }
      }
      if (flags != 63) {
        fprintf(stderr, "必须提供 "
                        "--source、--model、--input、--cache-read、--cache-"
                        "write、--output\n");
        return 1;
      }
      if (!set_model_pricing(source, model, input, read, write, output)) {
        fprintf(stderr, "价格配置失败，请确认费率非负。\n");
        return 1;
      }
  ...
  // "summary"
  // 命令：综合各个模块（使用、代码、Git、采纳率）的数据，做简洁快速的核心报表输出
  if (strcmp(subcommand, "summary") == 0) {
    ...
    if (!load_usage_stats(&usage) || !load_code_stats(&code) ||
        !load_git_stats(&git) || !load_attribution_stats(&attribution))
      return 1;
    printf("会话:       %ld\n", usage.total_sessions);
    printf("总 Token:   %ld（输入 %ld / 输出 %ld）\n",
           usage.input_tokens + usage.output_tokens, usage.input_tokens,
           usage.output_tokens);
    printf("工具调用:   %ld（MCP %ld）\n", usage.tool_calls, usage.mcp_calls);
    printf("代码变更:   %ld（新增 %ld / 删除 %ld）\n", code.change_events,
           code.lines_added, code.lines_deleted);
    printf("精确采纳率: %.1f%%（%ld / %ld 行）\n", attribution.acceptance_rate,
           attribution.accepted_lines, attribution.candidate_lines);
    printf("Git:        %ld 个仓库 / %ld 次提交\n", git.repositories,
           git.commits);
    AgentModelStats models[256];
    int model_count = load_model_stats(models, 256);
    double cost = 0;
    long priced = 0, calls = 0;
    if (model_count >= 0) {
      for (int i = 0; i < model_count; i++) {
        calls += models[i].model_calls;
        if (models[i].pricing_configured) {
          priced += models[i].model_calls;
          cost += models[i].estimated_cost_usd;
        }
      }
      if (priced > 0)
        printf("估算费用:   $%.4f（已配置价格覆盖 %ld / %ld 次调用）\n", cost,
               priced, calls);
      else
        printf("估算费用:   未配置（价格覆盖 0 / %ld 次调用）\n", calls);
    }
    return 0;
  }

  // "list"
  // 命令：列表显示最近发生的若干次会话摘要（包括时间、来源、调用的模型以及输入输出量等）
  if (strcmp(subcommand, "list") == 0) {
    ...
    if (limit > 100)
      limit = 100;
    AgentSessionStats sessions[100];
    int count = load_recent_session_stats(sessions, limit);
    if (count < 0)
      return 1;
    printf("%-20s %-12s %-28s %10s %10s %7s %7s\n", "时间", "Agent", "模型",
           "输入", "输出", "工具", "变更");
    for (int i = 0; i < count; i++)
      printf("%-20.20s %-12.12s %-28.28s %10ld %10ld %7ld %7ld\n",
             sessions[i].started_at, sessions[i].source,
             sessions[i].models[0] ? sessions[i].models : "未暴露",
             sessions[i].input_tokens, sessions[i].output_tokens,
             sessions[i].tool_calls, sessions[i].code_changes);
    return 0;
  }

  // "chart"
  // 命令：按控制台列表形式展示各个平台下不同模型的使用对比（调用频率、处理的
  // Token 总数）
  if (strcmp(subcommand, "chart") == 0) {
    AgentModelStats models[64];
    int count = load_model_stats(models, 64);
    if (count < 0)
      return 1;
    printf("%-12s %-32s %10s %12s %12s\n", "Agent", "模型", "调用/选择",
           "输入 Token", "输出 Token");
    for (int i = 0; i < count; i++)
      printf("%-12.12s %-32.32s %5ld/%-4ld %12ld %12ld\n", models[i].source,
             models[i].model, models[i].model_calls, models[i].selections,
             models[i].input_tokens, models[i].output_tokens);
    return 0;
  }

  // "record" 命令：手动通过命令行参数创建并录入一条新的 Agent
  // 执行记录，并实时计算预估 Token 花销
  if (strcmp(subcommand, "record") == 0) {
    ...
    rec.estimated_cost_usd = calculate_estimated_cost(
        rec.model, rec.input_tokens, rec.output_tokens);

    if (save_record(&rec)) {
      double rate =
          (rec.lines_suggested > 0)
              ? ((double)rec.lines_accepted / rec.lines_suggested) * 100.0
              : 0.0;
      printf("%s✓ Record saved successfully!%s\n", COLOR_GREEN, COLOR_RESET);
      printf("  Session ID:  %s%s%s\n", COLOR_BOLD, rec.session_id,
             COLOR_RESET);
      printf("  Project:     %s\n", rec.project);
      printf("  Model:       %s\n", rec.model);
      ...
      printf("  Acceptance:  %.1f%% (%d/%d lines)\n\n", rate,
             rec.lines_accepted, rec.lines_suggested);
    } else {
