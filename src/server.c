...
// 处理 /api/summary
// 接口请求，加载基础统计、代码统计和归因统计数据，并返回整体概况JSON
static void handle_api_summary(int client_fd) {
  ...
  if (!load_usage_stats(&usage) || !load_code_stats(&code) ||
    ...
    send_response(client_fd, "500 Internal Error", "application/json",
                  "{\"error\":\"failed to load real analytics\"}");
    return;
  }
...
static void handle_api_records(int client_fd) {
  ...
  if (!json_buf) {
    send_response(client_fd, "500 Internal Error", "text/plain",
...
static void handle_api_tools(int client_fd) {
  AgentToolStats rows[50];
  int count = load_tool_stats(rows, 50);
  if (count < 0) {
    send_response(client_fd, "500 Internal Error", "application/json",
...
static void handle_api_timeseries(int client_fd, const char *period) {
  ...
  if (count < 0) {
    send_response(
        ...
        "{\"error\":\"invalid period or failed to load time series\"}");
    return;
  }
...
static void handle_serve_html(int client_fd) {
  ...
  if (!fp) {
    ...
    const char *fallback_html =
        "<html lang=\"zh-CN\"><body><h1>AgentStat 服务已启动</h1><p>未找到 "
        "web/index.html。</p></body></html>";
...
int start_web_server(int port) {
  ...
  while (1) {
    ...
    if (strstr(buffer, "GET /api/summary") != NULL) {
      ...
      const char *period = strstr(buffer, "period=week")    ? "week"
                           : strstr(buffer, "period=month") ? "month"
                           : strstr(buffer, "period=day")   ? "day"
                                                            : "invalid";
