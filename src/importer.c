...
static bool query_text(sqlite3 *db, const char *json, const char *path,
                       char *out, size_t size) {
  sqlite3_stmt *stmt = NULL;
  const char *sql =
      "SELECT COALESCE(json_extract(?1,?2),'') WHERE json_valid(?1)";
...
static sqlite3_int64 query_int(sqlite3 *db, const char *json,
                               const char *path) {
  sqlite3_stmt *stmt = NULL;
  sqlite3_int64 value = 0;
  if (sqlite3_prepare_v2(
          db, "SELECT COALESCE(json_extract(?1,?2),0) WHERE json_valid(?1)", -1,
          &stmt, NULL) == SQLITE_OK) {
    sqlite3_bind_text(stmt, 1, json, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 2, path, -1, SQLITE_STATIC);
...
static const char *canonicalize_file_path(const char *path,
  ...
  while (ancestor[0]) {
    ...
    if (realpath(ancestor, resolved_ancestor)) {
      ...
      int written =
          snprintf(output, PATH_MAX, "%s%s", resolved_ancestor, suffix);
...
// 根据文件路径及后缀名对其代码类型进行归类 (generated, test, documentation,
// business, other)
static const char *classify_code_path(const char *path) {
  ...
  if (path_contains_segment(path, "/node_modules/") ||
      ...
      (extension &&
       (strcmp(extension, ".lock") == 0 || strcmp(extension, ".min.js") == 0)))
    return "generated";

  if (path_contains_segment(path, "/test/") ||
      path_contains_segment(path, "/tests/") ||
      path_contains_segment(path, "/__tests__/") ||
      path_contains_segment(path, "/spec/") || strstr(path, "_test.") ||
  ...
  if ((extension &&
      ...
      path_contains_segment(path, "/docs/") ||
      path_contains_segment(path, "/doc/"))
  ...
  if (extension) {
    ...
    for (size_t i = 0; i < count; i++)
      if (strcmp(extension, business_extensions[i]) == 0)
        return "business";
  }
  return "other";
}
...
static bool insert_agent_fingerprints(sqlite3 *db, const char *source_path,
                                      long line_number, const char *session_id,
                                      const char *timestamp,
                                      const char *file_path,
                                      const char *category, const char *diff) {
  const char *line = diff;
  long ordinal = 0;
  bool ok = true;
  while (ok && line && *line) {
    ...
    if (length > 1 && line[0] == '+' &&
      ...
      if (content_length > 0 && content[content_length - 1] == '\r')
        content_length--;
      if (content_length > 0) {
        char fingerprint[65];
        ...
        const char *sql = "INSERT OR IGNORE INTO agent_line_fingerprints"
                          "(source_path,line_number,file_path,line_ordinal,"
                          "session_id,timestamp,category,fingerprint)"
...
static bool import_code_changes(sqlite3 *db, const char *json,
  ...
  if (sqlite3_prepare_v2(db, query, -1, &changes, NULL) != SQLITE_OK)
    return false;
  ...
  while (ok && sqlite3_step(changes) == SQLITE_ROW) {
    ...
    const char *file_path =
        canonicalize_file_path(raw_file_path, canonical_file_path);
    ...
    const char *sql =
        ...
        "(source_path,line_number,session_id,turn_id,timestamp,file_path,"
        "change_type,category,lines_added,lines_deleted) "
    ...
    if (ok)
      ok = insert_agent_fingerprints(db, source_path, line_number, session_id,
                                     timestamp, file_path, category, diff);
  }
  sqlite3_finalize(changes);
  return ok;
}

// 核心函数：逐行解析 Codex 日志 (JSONL) 文件，提取并保存会话信息、Token
// 用量、工具调用及代码改动
bool import_codex_jsonl(const char *path, CodexImportResult *result) {
  ...
  while (ok && (line = read_jsonl_line(fp)) != NULL) {
    ...
    if (strcmp(outer_type, "session_meta") == 0) {
      ...
      if (!session_id[0])
        query_text(db, line, "$.payload.session_id", session_id,
                   sizeof(session_id));
      query_text(db, line, "$.payload.cwd", cwd, sizeof(cwd));
      query_text(db, line, "$.payload.model_provider", provider,
                 sizeof(provider));
      sqlite3_stmt *stmt = NULL;
      const char *sql = "INSERT INTO "
                        "sessions(session_id,source,source_path,cwd,started_at,"
                        "model_provider) "
                        "VALUES(?1,'codex',?2,?3,?4,?5) ON "
                        "CONFLICT(session_id) DO UPDATE SET "
                        "source_path=excluded.source_path,cwd=excluded.cwd,"
                        "model_provider=excluded.model_provider";
      ok = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) == SQLITE_OK;
      if (ok) {
        sqlite3_bind_text(stmt, 1, session_id, -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt, 2, source_path, -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt, 3, cwd, -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt, 4, timestamp, -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt, 5, provider, -1, SQLITE_TRANSIENT);
        ok = sqlite3_step(stmt) == SQLITE_DONE;
      }
      sqlite3_finalize(stmt);
      result->session_imported = ok;
    } else if (strcmp(outer_type, "turn_context") == 0) {
      query_text(db, line, "$.payload.model", current_model,
                 sizeof(current_model));
    } else if (session_id[0] && strcmp(outer_type, "event_msg") == 0 &&
               strcmp(payload_type, "token_count") == 0) {
      const char *names[] = {"input_tokens",
                             "cached_input_tokens",
      ...
      sqlite3_int64 cumulative_total_tokens =
          query_int(db, line, "$.payload.info.total_token_usage.total_tokens");
      ...
      for (int i = 0; i < 6; i++) {
        ...
        snprintf(total_path, sizeof(total_path),
                 "$.payload.info.total_token_usage.%s", names[i]);
        total_usage[i] = query_int(db, line, total_path);
        if (!has_total_snapshot || !has_previous_total_usage ||
            total_usage[i] != previous_total_usage[i])
      ...
      const char *sql = "INSERT OR IGNORE INTO model_usage_events "
                        "(source_path,line_number,session_id,timestamp,model,"
                        "input_tokens,cached_input_tokens,"
                        "cache_write_input_tokens,output_tokens,reasoning_"
      ...
      if (ok) {
        ...
        for (int i = 0; i < 6; i++) {
          char p[128];
          snprintf(p, sizeof(p), "%s%s", base, names[i]);
          sqlite3_bind_int64(stmt, 6 + i, query_int(db, line, p));
        }
        ok = sqlite3_step(stmt) == SQLITE_DONE;
        if (ok && sqlite3_changes(db) > 0)
      ...
      sqlite3_finalize(stmt);
               ...
               (strcmp(payload_type, "custom_tool_call") == 0 ||
                strcmp(payload_type, "function_call") == 0 ||
                strcmp(payload_type, "mcp_tool_call") == 0)) {
      char name[256] = "";
      query_text(db, line, "$.payload.name", name, sizeof(name));
      if (!name[0])
        query_text(db, line, "$.payload.tool", name, sizeof(name));
      int is_mcp = strcmp(payload_type, "mcp_tool_call") == 0 ||
                   strstr(name, "mcp__") == name;
      sqlite3_stmt *stmt = NULL;
      ok = sqlite3_prepare_v2(db,
                              "INSERT OR IGNORE INTO tool_calls "
                              "(source_path,line_number,session_id,timestamp,"
                              "tool_name,call_type,is_mcp,detail_name) "
      ...
      if (ok) {
        ...
        sqlite3_bind_text(stmt, 6, payload_type, -1, SQLITE_TRANSIENT);
        sqlite3_bind_int(stmt, 7, is_mcp);
        ok = sqlite3_step(stmt) == SQLITE_DONE;
        if (ok && sqlite3_changes(db) > 0)
      ...
      ok = import_code_changes(db, line, source_path, line_number, session_id,
                               timestamp, result);
    }
    free(line);
    line = NULL;
...
static bool sync_directory_recursive(const char *path,
  ...
  while ((entry = readdir(directory)) != NULL) {
    ...
    if (S_ISDIR(info.st_mode)) {
      if (!sync_directory_recursive(child_path, result))
        ok = false;
      ...
      result->duplicate_usage_events_skipped +=
          imported.duplicate_usage_events_skipped;
