...
// SHA-256 算法中使用的 64 个 32 位常量（由前 64 个质数的立方根的小数部分取前 32 位得到）
...
static uint32_t rotate_right(uint32_t value, uint32_t count) { return (value >> count) | (value << (32 - count)); }
...
// 初始化 SHA-256 上下文，设置初始的 8 个 32 位哈希值（由前 8 个质数的平方根的小数部分取前 32 位得到）
...
static void update(Sha256Context *context, const uint8_t *data, size_t length) {
    for (size_t i=0;i<length;i++) {
        ...
        if (context->data_length==64) { transform(context,context->data); context->bit_length+=512; context->data_length=0; }
    }
}
...
static void finish(Sha256Context *context, uint8_t digest[32]) {
    ...
    context->data[i++]=0x80;
    // 如果剩余空间不足以存放 64 位的长度信息（即已占用超过 56 字节），则先填充 0 并进行一次变换
    if (i>56) { while(i<64) context->data[i++]=0; transform(context,context->data); i=0; }
    // 填充 0，直到第 56 个字节
    while(i<56) context->data[i++]=0;
    // 计算消息的总位长度并追加到最后 8 个字节（大端序）
    context->bit_length += context->data_length * 8;
    for (int j=7;j>=0;j--) context->data[56+(7-j)]=(uint8_t)(context->bit_length>>(j*8));
    // 执行最后一次变换
    transform(context,context->data);
    // 将 8 个 32 位状态变量转换为 32 字节的输出结果（大端序）
    for (i=0;i<8;i++) { digest[i*4]=(uint8_t)(context->state[i]>>24); digest[i*4+1]=(uint8_t)(context->state[i]>>16); digest[i*4+2]=(uint8_t)(context->state[i]>>8); digest[i*4+3]=(uint8_t)context->state[i]; }
}

// 对外提供的便捷接口：计算数据的 SHA-256 哈希，并将结果转换为 64 个字符的十六进制字符串（包含结束符 '\0'）
void sha256_hex(const unsigned char *data, size_t length, char output[65]) {
    static const char hex[] = "0123456789abcdef";
    Sha256Context context; uint8_t digest[32];
    initialize(&context); update(&context,data,length); finish(&context,digest);
